########################################################################
## Mmodels - pCAR - FE (Whishart prior)
########################################################################
Mmodel_pcar_fe <-
        function(cmd = c("graph", "Q", "mu", "initial", "log.norm.const",
                ...
                if (!exists("cache.done", envir = envir)) {
                        ...
                        DiagD<- Matrix::Diagonal(x = D)
                        DiagD<- inla.as.sparse(DiagD)
                ...
                Q = function(){
                        ...
                        BlockIW <- Matrix::bdiag(lapply(1:k, function(i) {DiagD - param$alpha[i] * W }))
                        Q <- (MI %*% BlockIW) %*% t(MI)
                        Q <- inla.as.sparse(Q)
                        return (Q)
                }
...
########################################################################
## Mmodels - pCAR - RE (Whishart prior)
########################################################################
Mmodel_pcar_re <-
        function(cmd = c("graph", "Q", "mu", "initial", "log.norm.const",
                         "log.prior", "quit"), theta = NULL) {
